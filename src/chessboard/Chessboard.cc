                                   ...
                                   bool useOpenCV )
{
    ...
    {
        if ( findChessboardCornersImproved( image, patternSize, corners, flags ) )
            return true;
        else
                                           ...
                                           int flags )
{
    ...
    if ( patternSize.width > 127 || patternSize.height > 127 )
        return false;
    if ( patternSize.width == patternSize.height )
        return false;
...
// belong.
void
Chessboard::cleanFoundConnectedQuads( std::vector< ChessboardQuadPtr >& quadGroup, cv::Size patternSize )
{
...
void
Chessboard::labelQuadGroup( std::vector< ChessboardQuadPtr >& quadGroup, cv::Size patternSize, bool firstRun )
{
...
// own function
int
Chessboard::augmentBestRun( std::vector< ChessboardQuadPtr >& candidateQuads,
                            int candidateDilation,
                            std::vector< ChessboardQuadPtr >& existingQuads,
                            int existingDilation )
{
    // thresh dilation is used to counter the effect of dilation on the
    // distance between 2 neighboring corners. Since the distance below is
    // computed as its square, we do here the same. Additionally, we take the
...
//===========================================================================
void
Chessboard::generateQuads( std::vector< ChessboardQuadPtr >& quads, cv::Mat& image, int flags, int dilation, bool firstRun )
{
...
bool
less_pred( const std::pair< float, int >& p1, const std::pair< float, int >& p2 )
{
...
bool
Chessboard::checkBoardMonotony( std::vector< ChessboardCornerPtr >& corners, cv::Size patternSize )
{
...
bool
Chessboard::matchCorners( ChessboardQuadPtr& quad1, int corner1, ChessboardQuadPtr& quad2, int corner2 ) const
{
