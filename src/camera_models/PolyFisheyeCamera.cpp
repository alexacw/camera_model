...
PolyFisheyeCamera::PolyFisheyeCamera()
    : m_inv_K11(1.0), m_inv_K12(0.0), m_inv_K13(0.0), m_inv_K22(1.0), m_inv_K23(0.0)
{
    ...
    : mParameters(cameraName, imageWidth, imageHeight, k2, k3, k4, k5, k6, k7, p1, p2, A11, A12, A22, u0, v0, isFast)
{
    ...
    if (mParameters.isFast() == 1)
    {
        ...
        fastCalc = new FastCalcTABLE(coeff, mParameters.numDiff(), (double)mParameters.maxIncidentAngle());
    }
}
...
int PolyFisheyeCamera::imageWidth() const
{
    return mParameters.imageWidth();
}

int PolyFisheyeCamera::imageHeight() const
{
    return mParameters.imageHeight();
}

void PolyFisheyeCamera::spaceToPlane(const Eigen::Vector3d &P, Eigen::Vector2d &p) const
{
    double theta = acos(P(2) / P.norm());
    double inverse_r_P2 = 1.0 / sqrt(P(1) * P(1) + P(0) * P(0));
    Eigen::Vector2d dir(1, 0);
    if (isfinite(inverse_r_P2))
    {
        dir[1] = P(1) * inverse_r_P2;
        dir[0] = P(0) * inverse_r_P2;
    }
    ...
    else
    {
        ...
        // Apply generalised projection matrix
        p(0) = mParameters.A11() * theta * dir[0] /*p_u( 0 )*/ + mParameters.u0();
        p(1) = mParameters.A22() * theta * dir[1] /*p_u( 1 )*/ + mParameters.v0();
    }
}

void PolyFisheyeCamera::spaceToPlane(const Eigen::Vector3d &P, Eigen::Vector2d &p, float image_scalse) const
{
 ...
 */
void PolyFisheyeCamera::spaceToPlane(const Eigen::Vector3d &P,
                                     Eigen::Vector2d &p,
...
void PolyFisheyeCamera::estimateIntrinsics(const cv::Size &boardSize,
...
void PolyFisheyeCamera::liftSphere(const Eigen::Vector2d &p, Eigen::Vector3d &P) const
{
    liftProjective(p, P);
}

void PolyFisheyeCamera::rayToPlane(const Ray &ray, Eigen::Vector2d &p) const
{
...
void PolyFisheyeCamera::liftProjectiveToRay(const Eigen::Vector2d &p, Ray &ray) const
{
    // TODO
...
void PolyFisheyeCamera::liftProjective(const Eigen::Vector2d &p, Eigen::Vector3d &P) const
{
...
void PolyFisheyeCamera::liftProjective(const Eigen::Vector2d &p, Eigen::Vector3d &P, float image_scale) const
{
...
void PolyFisheyeCamera::undistToPlane(const Eigen::Vector2d &p_u, Eigen::Vector2d &p) const
{
}
...
int PolyFisheyeCamera::parameterCount() const
{
...
void PolyFisheyeCamera::setParameters(const PolyFisheyeCamera::Parameters &parameters)
{
...
void PolyFisheyeCamera::readParameters(const std::vector<double> &parameterVec)
{
...
void PolyFisheyeCamera::writeParameters(std::vector<double> &parameterVec) const
{
...
void PolyFisheyeCamera::backprojectSymmetric(const Eigen::Vector2d &p_u,
                                             double &cos_theta,
                                             double &sin_theta,
                                             double &cos_phi,
...
bool PolyFisheyeCamera::calcKinvese(double a11, double a12, double a22, double u0, double v0)
{
...
void PolyFisheyeCamera::setPoly(math_utils::Polynomial *value)
{
    poly = value;
}
...
void PolyFisheyeCamera::setFastCalc()
{
    ...
    fastCalc = new FastCalcTABLE(coeff_fast, mParameters.numDiff(), (double)mParameters.maxIncidentAngle());
...
PolyFisheyeCamera::Parameters::Parameters()
    : Camera::Parameters(POLYFISHEYE), m_k2(0.0), m_k3(0.0), m_k4(0.0), m_k5(0.0), m_k6(0.0), m_k7(0.0), m_p1(0.0), m_p2(0.0), m_A11(1.0), m_A12(0.0), m_A22(1.0), m_u0(0.0), m_v0(0.0), m_isFast(0), m_numDiff(FAST_NUM_DEFAULT), m_maxIncidentAngle(FAST_MAX_INCIDENT_ANGLE)
{
}
...
PolyFisheyeCamera::Parameters::Parameters(const std::string &cameraName,
                                          ...
                                          double v0,
                                          int isFast)
    : Camera::Parameters(POLYFISHEYE, cameraName, w, h), m_k2(k2), m_k3(k3), m_k4(k4), m_k5(k5), m_k6(k6), m_k7(k7), m_p1(p1), m_p2(p2), m_A11(A11), m_A12(A12), m_A22(A22), m_u0(u0), m_v0(v0), m_isFast(isFast), m_numDiff(FAST_NUM_DEFAULT), m_maxIncidentAngle(FAST_MAX_INCIDENT_ANGLE)
{
}
...
int &PolyFisheyeCamera::Parameters::numDiff()
{
    return m_numDiff;
}

int &PolyFisheyeCamera::Parameters::maxIncidentAngle()
{
    return m_maxIncidentAngle;
}

int &PolyFisheyeCamera::Parameters::isFast()
{
    return m_isFast;
}
...
int PolyFisheyeCamera::Parameters::isFast() const
{
    return m_isFast;
}

int PolyFisheyeCamera::Parameters::numDiff() const
{
    return m_numDiff;
}

int PolyFisheyeCamera::Parameters::maxIncidentAngle() const
{
    return m_maxIncidentAngle;
}
...
bool PolyFisheyeCamera::Parameters::readFromYamlFile(const std::string &filename)
{
...
bool PolyFisheyeCamera::Parameters::isDistortion() const
{
    return (m_k2 != 0.0);
}

void PolyFisheyeCamera::Parameters::writeToYamlFile(const std::string &filename) const
{
...
void PolyFisheyeCamera::FastCalcTABLE::setMaxIncidentAngle(double value)
{
    maxIncidentAngle = value;
    resetFastCalc();
}

void PolyFisheyeCamera::FastCalcTABLE::setNumDiffAngle(int value)
{
    numDiffAngle = value;
    resetFastCalc();
}

void PolyFisheyeCamera::FastCalcTABLE::setMaxImageR(double value)
{
    maxImageR = value;
    resetFastCalc();
}

void PolyFisheyeCamera::FastCalcTABLE::setNumDiffR(int value)
{
    numDiffR = value;
    resetFastCalc();
}
...
int PolyFisheyeCamera::FastCalcTABLE::getNumDiff()
{
    return numDiffR;
}
...
bool PolyFisheyeCamera::FastCalcTABLE::calcAngleToR(eigen_utils::Matrix &_angleToR,
...
bool PolyFisheyeCamera::FastCalcTABLE::calcRToAngle(eigen_utils::Matrix &_rToAngle,
                                                    const int _numDiffR,
                                                    const double _diffR,
...
void PolyFisheyeCamera::FastCalcTABLE::backprojectSymmetric(const Eigen::Vector2d &p_u,
                                                            double &cos_theta,
                                                            double &sin_theta,
                                                            double &cos_phi,
                                                            double &sin_phi) const
...
void PolyFisheyeCamera::FastCalcTABLE::resetFastCalc()
{
...
void PolyFisheyeCamera::FastCalcPOLY::backprojectSymmetric(const Eigen::Vector2d &p_u,
                                                           double &cos_theta,
                                                           double &sin_theta,
                                                           double &cos_phi,
                                                           double &sin_phi) const
...
void PolyFisheyeCamera::FastCalcPOLY::setMaxIncidentAngle(double value)
{
    maxIncidentAngle = value;
    resetFastCalc();
}

void PolyFisheyeCamera::FastCalcPOLY::setMaxImageR(double value)
{
    maxImageR = value;
    resetFastCalc();
}

void PolyFisheyeCamera::FastCalcPOLY::resetFastCalc()
{
